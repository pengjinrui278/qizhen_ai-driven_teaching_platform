...
团队已选择先做本地开发者预览。仓库新增 `apps/zju-connector`：服务固定绑定 `127.0.0.1`，账号密码只用于本次本机 CAS 请求且不落盘，Cookie 仅保存在进程内存；首批读取本科教务课表和“学在浙大”DDL，导出时剔除学号、密码、Cookie 与原始响应。该实现降低了云端集中保管凭据的风险，但不改变本报告关于公开分发、内部接口授权与校外部署备案的结论。
