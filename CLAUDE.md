...
1. **重大更新必须推送并上线**：里程碑完成、能力实质变化、语料批次扩充、申报材料定稿、以及任何影响评审展示的改动，提交并推送到 `origin/main`，同步部署到生产 `http://124.220.5.87/`（脚本 `scripts/deploy-prod.sh`），同时在 `docs/progress/进度日志.md` 追加条目。
