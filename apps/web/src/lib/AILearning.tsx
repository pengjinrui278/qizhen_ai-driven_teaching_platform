...
export default function AILearning({section=""}:{section?:string}){
 ...
 {tab==="setup"&&<><p>选择工具，按官方指引在自己的电脑安装。</p><div className="aiGrid">{installers.map(t=><article className="portalPanel" key={t.name}><h2>{t.name}</h2><ol>{t.steps.map(s=><li key={s}>{s}</li>)}</ol><p>验证：<code>{t.verify}</code></p><a href={t.url} target="_blank" rel="noopener noreferrer">打开安装文档 ↗</a></article>)}</div><section className="portalPanel"><h2>接入与安全</h2><p>在工具本地配置服务商、接口地址、模型和密钥。学镜学习空间的 DeepSeek 密钥不会分发给个人工具；并非所有工具都支持同一种接口。</p><p>密钥不要贴进聊天、截图或代码仓库。安装失败先核对系统要求、网络和官方错误说明；卸载使用系统应用管理或官方卸载指引，先备份项目和配置。</p></section></>}
 {noteEditor}
 </div>;
}
